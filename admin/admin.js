(async function(){
  ...
  favicon.href='../assets/favicon.svg?v=20260903';
  ...
  async function savePost(e){e.preventDefault();const status=$('blog-form-status');if(!status)return;status.textContent='Salvataggio…';const payload={id:$('blog-id')?.value||undefined,title:$('blog-title')?.value||'',category:$('blog-category')?.value||'',content:$('blog-content')?.innerHTML.trim()||'',cover_image_url:$('blog-cover')?.value||'',meta_description:$('blog-meta')?.value||'',is_published:$('blog-is-published')?.checked||false};try{const saved=await api('/api/blog',{method:payload.id?'PUT':'POST',body:JSON.stringify(payload)});const i=state.blog.findIndex(x=>x.id===saved.id);if(i>=0)state.blog[i]=saved;else state.blog.unshift(saved);renderBlog();status.textContent='Articolo salvato';setStatus('Backend collegato')}catch(e){status.textContent=e.message;setStatus('Errore backend',true)}}
