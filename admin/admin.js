(async function(){
  ...
  function renderBlog(){const list=$('blog-list');if(!list)return;list.innerHTML=state.blog.length?state.blog.map(p=>`<div class="managed-item"><div><strong>${esc(p.title)}</strong><small>${esc(p.category||'Senza categoria')} · ${p.is_published?'Pubblicato':'Bozza'}</small></div><div class="managed-actions"><button class="mini" data-edit-post="${p.id}">Modifica</button><button class="mini" data-delete-post="${p.id}">Elimina</button></div></div>`).join(''):'<div class="form-card note">Nessun articolo presente.</div>';$('count-blog').textContent=state.blog.filter(p=>p.is_published).length}
  function resetPostForm(){$('blog-form').reset();$('blog-id').value='';$('blog-content').innerHTML='';$('blog-form-status').textContent=''}
  function fillPostForm(p){$('blog-id').value=p.id;$('blog-title').value=p.title||'';$('blog-category').value=p.category||'';$('blog-content').innerHTML=p.content||'';$('blog-cover').value=p.cover_image_url||'';$('blog-meta').value=p.meta_description||'';$('blog-is-published').checked=Boolean(p.is_published);activatePanel('blog')}
  ...
  async function savePost(e){e.preventDefault();const status=$('blog-form-status');status.textContent='Salvataggio…';const payload={id:$('blog-id').value||undefined,title:$('blog-title').value,category:$('blog-category').value,content:$('blog-content').innerHTML.trim(),cover_image_url:$('blog-cover').value,meta_description:$('blog-meta').value,is_published:$('blog-is-published').checked};try{const saved=await api('/api/blog',{method:payload.id?'PUT':'POST',body:JSON.stringify(payload)});const i=state.blog.findIndex(x=>x.id===saved.id);if(i>=0)state.blog[i]=saved;else state.blog.unshift(saved);renderBlog();status.textContent='Articolo salvato';setStatus('Backend collegato')}catch(e){status.textContent=e.message;setStatus('Errore backend',true)}}
