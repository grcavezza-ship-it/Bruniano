(() => {
  ...
  async function moveMedia(id,slot){const item=state.items.find(x=>x.id===id);if(!item)return;try{const saved=await api('/api/gallery',{method:'PUT',body:JSON.stringify({...item,studio_slot:slot,is_published:slot !== null}));state.items[state.items.findIndex(x=>x.id===id)]=saved;renderSlots();renderCatalog();}catch(error){alert(error.message);}}
