(() => {
  ...
  topbar.append(account, accountMenu);
  ...
  function syncAccount() {
    ...
    if (target) target.textContent = (avatar?.textContent || 'A').trim().slice(0, 2) || 'A';
  }
  ...
  account.addEventListener('click', event => {
    event.stopPropagation();
    accountMenu.classList.contains('is-visible') ? closeAccount() : openAccount();
  });

  document.addEventListener('click', event => {
    if (!accountMenu.contains(event.target) && event.target !== account) closeAccount();
  });
  ...
  const avatar = document.getElementById('user-avatar');
  if (avatar) new MutationObserver(syncAccount).observe(avatar, { childList: true, characterData: true, subtree: true });
  syncAccount();
})();
