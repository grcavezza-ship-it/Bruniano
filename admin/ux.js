(() => {
  ...
  style.textContent = `
    ...
    @media(max-width:600px){.media-admin-grid{grid-template-columns:1fr}.admin-logo strong{font-size:22px}.admin-logo img{width:38px;height:38px}.gallery-toolbar{align-items:flex-start;flex-direction:column}.form-row{grid-template-columns:1fr}.admin-header{height:auto;min-height:70px;padding:12px 16px}}
  `;
  ...
  async function mediaAction(url,method,id){
    ...
    const r=await fetch(url,{method,headers:{'content-type':'application/json'},body:JSON.stringify(body)}); if(!r.ok)throw new Error('Operazione non riuscita'); return r;
  }
