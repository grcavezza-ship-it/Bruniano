...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === 'POST') {
      ...
      const rows = await sql`INSERT INTO blog_posts (slug,title,excerpt,content,cover_image_url,category,meta_description,author,published_at,is_published) VALUES (${slugify(body.slug || title)},${title},${body.excerpt || null},${content},${body.cover_image_url || null},${body.category || null},${body.meta_description || null},${body.author || null},${body.published_at || null},${Boolean(body.is_published)}) RETURNING *`;
      return send(res, rows[0], 201);
    }
    if (req.method === 'PUT') {
      const id = body.id, title = String(body.title || '').trim(), content = String(body.content || '').trim();
      if (!id) return send(res, { error: 'ID mancante' }, 400);
      if (!title || !content) return send(res, { error: 'Titolo e contenuto sono obbligatori' }, 400);
      const rows = await sql`UPDATE blog_posts SET slug=${slugify(body.slug || title)},title=${title},excerpt=${body.excerpt || null},content=${content},cover_image_url=${body.cover_image_url || null},category=${body.category || null},meta_description=${body.meta_description || null},author=${body.author || null},published_at=${body.published_at || null},is_published=${Boolean(body.is_published)},updated_at=now() WHERE id=${id} RETURNING *`;
