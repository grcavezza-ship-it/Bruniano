...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === 'POST' || req.method === 'PUT') {
      ...
      if (!ALLOWED_TYPES.has(mediaType)) return send(res, { error: 'Tipo media non consentito' }, 400);
      if (mediaUrl.length > 2048 || altText.length > 300 || title.length > 200) return send(res, { error: 'Dati media troppo lunghi' }, 400);
      if (id) {
